<template>
    ...
    <div class="w-full bg-white rounded-md px-6 py-3 flex justify-start items-center gap-3 shadow-lg">
        ...
        <RouterLink :to="{name:'lessons'}" class="decoration-0 text-black inline-block font-medium text-md">
            Lessons
        </RouterLink>
    </div>
    <!-- / breadcrumb -->
                ...
                <div class="w-full block">
                    ...
                    <div class="mb-3 w-full block">
                        ...
                        <div class="w-full relative">
                            <select name="section_id" id="section_id" v-model="formData.section_id" class="form-select" autocomplete="off">
                                <option :value="null"> Select Section </option>
                                <option v-for="(each,index) in sectionData" :key="index" :value="each.id"> {{each.title}} </option>
                            </select>
                        ...
                        <div class="mt-2 w-full block text-red-500 text-xs font-medium" v-if="error?.section_id"> {{error?.section_id[0]}} </div>
                    </div>
                    <!-- / section -->
                    ...
                    <div class="mb-3 w-full block">
                        ...
                        <input id="video_url" type="url" name="video_url" v-model="formData.video_url" class="form-control" autocomplete="off" />
                        <div class="mt-2 w-full block text-red-500 text-xs font-medium" v-if="error?.video_url"> {{error?.video_url[0]}} </div>
                    </div>
                    <!-- / video url -->
                    ...
                    <div class="mb-3 w-full block">
                        ...
                        <input id="duration" type="text" name="duration" v-model="formData.duration" class="form-control" autocomplete="off" />
                        <div class="mt-2 w-full block text-red-500 text-xs font-medium" v-if="error?.duration"> {{error?.duration[0]}} </div>
                    </div>
                    <!-- / duration -->
    ...
    methods: {
        ...
        async listApi() {
            try {
                ...
                const response = await axios.get(apiRoutes.lesson.list, {params: this.params}, {headers: apiServices.headerContent});
        ...
        async showApi(data) {
            try {
                ...
                const response = await axios.get(apiRoutes.lesson.show(data), { headers: apiServices.headerContent });
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoutes.lesson.delete(this.formData.id), { headers: apiServices.headerContent });
                await this.listApi();
