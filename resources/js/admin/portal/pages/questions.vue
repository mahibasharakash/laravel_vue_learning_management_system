<template>
    ...
    <div class="w-full bg-white rounded-md px-6 py-3 flex justify-start items-center gap-3 shadow-lg">
        ...
        <RouterLink :to="{name:'questions'}" class="decoration-0 text-black inline-block font-medium text-md">
            Questions
        </RouterLink>
    </div>
    <!-- / breadcrumb -->
    ...
    <div class="w-full mt-3">
            ...
            <div class="w-full min-h-[calc(100vh-300px)] max-h-[calc(100vh-300px)] rounded-md shadow-lg bg-white p-2.5 overflow-x-auto">
                <table class="table table-auto w-full">
                    ...
                    <tr class="w-full bg-transparent duration-500 hover:bg-blue-50 overflow-hidden" v-for="(each, index) in tableData" :key="index">
                        ...
                        <td class="min-w-[250px] max-w-[250px] py-3 px-3.5 text-gray-800 text-start text-sm font-medium">
                            <template v-if="each.type === 'mcq'">
                                MCQ
                            </template>
                            <template v-if="each.type === 'true_false'">
                                True Or False
                            </template>
                            <template v-if="each.type === 'short_answer'">
                                Short Answer
                            </template>
                        </td>
        ...
        <!-- no data found -->
        <emptyPage message="No Questions Available" subMessage="question" @add="openManageModal(null)" v-if="tableData.length === 0 && !listLoading"></emptyPage>
        <!-- / no data found -->
            ...
            <form @submit.prevent="manageApi()" class="w-full block">
                ...
                <div class="w-full block">
                    ...
                    <div class="mb-3 w-full block">
                        ...
                        <div class="w-full relative">
                            <select name="quiz_id" id="quiz_id" v-model="formData.quiz_id" class="form-select" autocomplete="off">
                                ...
                                <option v-for="(each, index) in quizData" :value="each.id"> {{each.title}} </option>
                            </select>
                    ...
                    <div class="mb-3 w-full block">
                        ...
                        <input id="question" type="text" name="question" v-model="formData.question" class="form-control" autocomplete="off" />
                        <div class="mt-2 w-full block text-red-500 text-xs font-medium" v-if="error?.question"> {{error?.question[0]}} </div>
                    </div>
                    <!-- / question -->
    ...
    methods: {
        ...
        async listApi() {
            try {
                ...
                const response = await axios.get(apiRoutes.question.list, {params: this.params}, {headers: apiServices.headerContent});
        ...
        async createApi() {
            try {
                ...
                formData.append('quiz_id', this.formData.quiz_id);
                formData.append('question', this.formData.question);
                formData.append('type', this.formData.type);
                await axios.post(apiRoutes.question.store, formData, { headers: apiServices.headerContent });
                this.formData = { id: '', quiz_id: '', question: '', type: '' };
                await this.listApi();
        ...
        async updateApi() {
            try {
                ...
                await axios.post(apiRoutes.question.update(this.formData.id), formData, { headers: apiServices.mediaHeaderContent });
        ...
        async showApi(data) {
            try {
                ...
                const response = await axios.get(apiRoutes.question.show(data), { headers: apiServices.headerContent });
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoutes.question.delete(this.formData.id), { headers: apiServices.headerContent });
                await this.listApi();
