<template>
    ...
    <div class="w-full bg-white rounded-md px-6 py-3 flex justify-start items-center gap-3 shadow-lg">
        ...
        <RouterLink :to="{name:'enrollments'}" class="decoration-0 text-black inline-block font-medium text-md">
            Enrollments
        </RouterLink>
    </div>
    <!-- / breadcrumb -->
    ...
    <div class="w-full mt-3">
        ...
        <!-- no data found -->
        <emptyPage message="No Enrollments Available" subMessage="enrollment" @add="openManageModal(null)" v-if="tableData.length === 0 && !listLoading"></emptyPage>
        <!-- / no data found -->
            ...
            <form @submit.prevent="manageApi()" class="w-full block">
                ...
                <div class="w-full block">
                    ...
                    <div class="mb-3 w-full block">
                        ...
                        <div class="w-full relative">
                            <select name="user_id" id="user_id" v-model="formData.user_id" class="form-select" autocomplete="off">
                                ...
                                <option v-for="(each, index) in userData" :key="index" :value="each.id"> {{each.name}} </option>
                            </select>
                    ...
                    <div class="mb-3 w-full block">
                        ...
                        <input id="progress" type="text" name="progress" v-model="formData.progress" class="form-control" autocomplete="off" />
                        <div class="mt-2 w-full block text-red-500 text-xs font-medium" v-if="error?.progress"> {{error?.progress[0]}} </div>
                    </div>
                    <!-- / progress -->
    ...
    methods: {
        ...
        async listApi() {
            try {
                ...
                const response = await axios.get(apiRoutes.enrollment.list, {params: this.params}, {headers: apiServices.headerContent});
        ...
        async createApi() {
            try {
                ...
                formData.append('progress', this.formData.progress);
                await axios.post(apiRoutes.enrollment.store, formData, { headers: apiServices.headerContent });
                this.formData = { id: '', user_id: '', course_id: '', progress: '' };
                await this.listApi();
        ...
        async updateApi() {
            try {
                ...
                await axios.post(apiRoutes.enrollment.update(this.formData.id), formData, { headers: apiServices.mediaHeaderContent });
                this.formData = { id: '', user_id: '', course_id: '', progress: '' };
                await this.listApi();
                this.closeManageModal();
            } catch (e) {
        ...
        async showApi(data) {
            try {
                ...
                const response = await axios.get(apiRoutes.enrollment.show(data), { headers: apiServices.headerContent });
        ...
        async deleteApi() {
            try {
                ...
                await axios.delete(apiRoutes.enrollment.delete(this.formData.id), { headers: apiServices.headerContent });
                await this.listApi();
